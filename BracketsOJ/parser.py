...
class Parser:
    ...
    def Jmp_X(self, String):
        ...
        if String == "jmp":
            ...
            if self.Flags["sf"] == 0 and self.Flags["of"] == 0:
                ...
                return True
            return False
        elif (String == "jbe") or (String == "jna") or (String == "jle") or (String == "jng"):
            if self.Flags["sf"] != self.Flags["of"]:
                return True
            elif self.Flags["zf"] == 1:
                return True
    ...
    def Cmp(self, String, infix):
        ...
        if ((tmp1[0] == 'add') and (tmp2[0] == 'add')) or (
            ...
            return False
        a = 0
        if (tmp1[0] != 'var') and (tmp1[0] != 'add'):
            a = tmp1[1]
        else:
            a = self.Get_value_from_memory(tmp1[1], tmp1[2])
        b = 0
        if (tmp2[0] != 'var') and (tmp2[0] != 'add'):
            b = tmp2[1]
        else:
            b = self.Get_value_from_memory(tmp2[1], tmp2[2])
    ...
    def Xchg(self, String, infix):
        ...
        if (tmp1[0] == 'imm') or (tmp1[2] == 0) or ((tmp1[0] == 'imm') and (tmp2[0] == 'imm')):
            return False
        if ((tmp1[0] == 'add') and (tmp2[0] == 'add')) or ((tmp1[0] == 'imm') and (tmp2[0] == 'imm')) or (
                (tmp1[2] != tmp2[2]) and (tmp2[2] != 0)):
            return False
        ...
        if tmp2[0] == 'reg':
            ...
            if not self.Save_value_in_memory(tmp2[1], a, tmp2[2]):
                return False
        return True
    ...
    def Shift(self, String, infix):
        ...
        if (tmp1 is False) or (tmp2 is False):
            return False
        if ((tmp1[0] == 'reg') or (tmp1[0] == 'var') or ((tmp1[0] == 'add') and (tmp1[2] != 0))) and (
                ((tmp2[0] == 'imm' and tmp2[2] == 1)) or ((tmp2[0] == 'reg') and (infix[1][0] == 'cl'))):
            # "", "","", "", "rol", "ror", "rcl", "rcr"
    ...
    def Check_code_operand(self, Operand):
        if len(Operand) == 1:
            ...
            elif Operand[0] in self.Registers:
                ...
                if (Operand[i] == '+') or (Operand[i] == '-') or (Operand[i] == '*') or (Operand[i] == '/'):
                    ...
                    if len(stak) > 0:
                        ...
                        if (tmp1[0] != "var") and ((Operand[i] == 'lengthof') or (Operand[i] == 'sizeof')):
                            return False
                        elif (tmp1[0] != "var") and (Operand[i] == 'type'):
                            stak[len(stak) - 1] = 0
                        else:
    ...
    def Start_Code(self):
        if len(self.Code_Lines[len(self.Code_Lines) - 1]) == 2:
            if (self.Code_Lines[self.Code_Lines.__len__() - 1][0] == "end") and (
                    (self.Code_Lines[len(self.Code_Lines) - 1][1]) in self.Functions_names):
                ...
                while self.Registers["eip"] < len(self.Code_segment):
                    ...
                    if (self.Code_segment[self.Registers["eip"]] == "") and (
                            self.Search_lable(self.Registers["eip"]) == False):
                        return False
                    ...
                    if self.Code_segment[self.Registers["eip"]] == "":
                        ...
                        if self.Code_segment[self.Registers["eip"]] == "exit":
                            ...
                            if self.Use_Uses.__len__() == 0:
                                ...
                                b = 20
                    elif self.Special_Names_one_Operands.__contains__(
                            self.Code_segment[self.Registers["eip"]][0]):

                        if (self.Code_segment[self.Registers["eip"]][0][0] == 'j') or (
                                self.Code_segment[self.Registers["eip"]][0][0] == 'l'):
                            tmp = self.Jmp_X(self.Code_segment[self.Registers["eip"]][0])
                            if tmp:
                                self.Registers.update(
                                    {"eip": self.Labels_names[self.Code_segment[self.Registers["eip"]][1]]})
                                continue
                        elif (self.Code_segment[self.Registers["eip"]][0] == 'mul') or (
                                self.Code_segment[self.Registers["eip"]][0] == 'imul'):
                            if not self.Mul_X(self.Code_segment[self.Registers["eip"]][0],
                                              self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif (self.Code_segment[self.Registers["eip"]][0] == 'div') or (
                                self.Code_segment[self.Registers["eip"]][0] == 'idiv'):
                            if not self.Div_X(self.Code_segment[self.Registers["eip"]][0],
                                              self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif self.Code_segment[self.Registers["eip"]][0][0] == 'p':
                            if not self.Push_Pop(self.Code_segment[self.Registers["eip"]][0],
                                                 self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif (self.Code_segment[self.Registers["eip"]][0] == 'neg') or (
                                self.Code_segment[self.Registers["eip"]][0] == 'inc') or (
                                self.Code_segment[self.Registers["eip"]][0] == 'dec'):
                            if not self.Neg_inc_dec(self.Code_segment[self.Registers["eip"]][0],
                                                    self.Code_segment[self.Registers["eip"]][1]):
                                return False
                            ...
                            if self.Functions_names.__contains__(self.Code_segment[self.Registers["eip"]][1]):
                                ...
                                self.Registers.update({"esp": self.Registers["esp"] + 1})
                                self.Registers.update(
                                    {"eip": self.Functions_names[self.Code_segment[self.Registers["eip"]][1]]})
                            else:
                                if not self.Irvine32(self.Code_segment[self.Registers["eip"]][1]):
                                    return False


                    elif self.Special_Names_two_Operands.__contains__(
                            self.Code_segment[self.Registers["eip"]][0]):
                        L1 = ["add", "sub", "sbb", "acd"]
                        L2 = ["test", "xor", "and", "or"]
                        ...
                        if self.Code_segment[self.Registers["eip"]][0][0] == 'm':
                            if not self.Mov_X(self.Code_segment[self.Registers["eip"]][0],
                                              self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif self.Code_segment[self.Registers["eip"]][0][0] == 'c':
                            if not self.Cmp(self.Code_segment[self.Registers["eip"]][0],
                                            self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif self.Code_segment[self.Registers["eip"]][0] == 'xchg':
                            if not self.Xchg(self.Code_segment[self.Registers["eip"]][0],
                                             self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif L1.__contains__(self.Code_segment[self.Registers["eip"]][0]):
                            if not self.Add_sub(self.Code_segment[self.Registers["eip"]][0],
                                                self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif L2.__contains__(self.Code_segment[self.Registers["eip"]][0]):
                            if not self.Test(self.Code_segment[self.Registers["eip"]][0],
                                             self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif L3.__contains__(self.Code_segment[self.Registers["eip"]][0]):
                            if not self.Mem_opr(self.Code_segment[self.Registers["eip"]][0],
                                                self.Code_segment[self.Registers["eip"]][1]):
                                return False
                        elif L4.__contains__(self.Code_segment[self.Registers["eip"]][0]):
                            if not self.Shift(self.Code_segment[self.Registers["eip"]][0],
                                              self.Code_segment[self.Registers["eip"]][1]):
                                return False
        ...
        for i in range(0, len(Line)):
            ...
            if (Line[i] == '(') or (Line[i] == '['):
                ...
                if len(stak) > 0:
                    ...
                    if (Line[i] == '[') and (
                            (stak[len(stak) - 1] == "lengthof") or (stak[len(stak) - 1] == "sizeof") or (
                            stak[len(stak) - 1] == "type") or (stak[len(stak) - 1] == "offset")):
                        return False
                    if (Line[i] == '(') and (
                            (stak[len(stak) - 1] == "lengthof") or (stak[len(stak) - 1] == "sizeof") or (
                            stak[len(stak) - 1] == "offset")):
                        return False
                if (len(stak) == 0) and (Line[i] == '(') and (expression.__len__() != 0):
                    return False
                if expression.__len__() > 0:
                    if (Line[i] == '[') and ((expression[expression.__len__() - 1]) != "ptr") and (
                            (reg_32.__contains__(expression[expression.__len__() - 1]) == False) and (
                            self.Data_variables.__contains__(expression[expression.__len__() - 1]) == False)):
                        return False
                    elif (Line[i] == '[') and ((expression[expression.__len__() - 1]) != "ptr") and (
                            (reg_32.__contains__(expression[expression.__len__() - 1]) == False)):
                ...
                if (Line[i] == '*') or (Line[i] == '-') or (Line[i] == '/') or (Line[i] == '+'):
                    if len(stak) > 0:
                        ...
                        while j >= 0:
                            if ((stak[j] == '+') or (stak[j] == '-')) and ((Line[i] == '+') or (Line[i] == '-')):
                                ...
                                expression.append(stak[j])
                                stak = stak[:-1]
                            elif ((stak[j] == 'dup') or (stak[j] == 'lengthof') or (stak[j] == 'type') or (
                                    stak[j] == 'sizeof')):
                                expression.append(stak[j])
                                stak = stak[:-1]
                            else:
                    ...
                    try:
                        if ((Line[i][0] == Line[i][len(Line[i]) - 1]) and (Line[i][0] == '"')) or (
                                (Line[i][0] == Line[i][len(Line[i]) - 1]) and (Line[i][0] == "\'")):
    ...
    def Build_code_segment(self):
        ...
        while i < len(self.Code_Lines) - 1:
            ...
            if (len(self.Code_Lines[i]) == 1) and (self.Code_Lines[i][0] == ".code"):
                ...
                if (len(self.Code_Lines[i]) == 1) and (
                        self.Code_Lines[i][0] in self.Special_Names_no_Operands):
                    ...
                    if len(self.Code_Lines[i]) == 2:
                        if self.Code_Lines[i][1] == 'proc':
                            ...
                            if (self.Code_Lines[i][0] == 'call') and (
                                    (self.Code_Lines[i][1] in self.Functions_names) or (
                                    self.Code_Lines[i][1] in self.Irvine32_functions)):
                                self.Code_segment.append(self.Code_Lines[i])
                            elif ((self.Code_Lines[i][0][0] == 'j') or (self.Code_Lines[i][0][0] == 'l') or (
                                    self.Code_Lines[i][0][0] == 'r')) and (self.Code_Lines[i][1] in self.Labels_names):
                                self.Code_segment.append(self.Code_Lines[i])
                            elif ((self.Code_Lines[i][0] != 'call') and (self.Code_Lines[i][0][0] != 'j') and (
                                    self.Code_Lines[i][0][0] != 'l') and (self.Code_Lines[i][0][0] != 'r')):
                            ...
                            if self.Opened_function != "":
                                for j in range(1, len(self.Code_Lines[i])):
                                    if (reg_32.__contains__(self.Code_Lines[i][j]) == False) and (
                                            reg_16.__contains__(self.Code_Lines[i][j]) == False):
                                        return False
                                self.Code_segment.append(self.Code_Lines[i])
                            else:
                                return False
        ...
        if not Comma:
                    ...
                    if infix:
                        ...
                        for i in range(0, len(infix)):
                            ...
                            for j in range(0, len(infix[i])):
                                if (infix[i][j] == '+') or (infix[i][j] == '-') or (infix[i][j] == '*') or (
                                        infix[i][j] == '/'):
                                    if len(stak) > 1:
                                        ...
                                        if tmp1 == -2:
                                            tmp1 = stak[len(stak) - 1]
                                        elif tmp1 == -3:

                                            tmp1 = extra_functions.convert_string(stak[len(stak) - 1])
                                    ...
                                    if len(stak) > 0:
                                        tmp = self.Check_is_valid_data(stak[len(stak) - 1])
                                        if (((tmp == 0) or (tmp == -1) or (tmp == -2) or (tmp == -3)) and (
                                                (infix[i][j] == 'lengthof') or (infix[i][j] == 'sizeof'))):
                                            return 0
                                        elif ((tmp == 0) or (tmp == -1) or (tmp == -2) or (tmp == -3)) and (
                                                infix[i][j] == 'type'):
                                            stak[len(stak) - 1] = 0
                                        else:
                                            stak = stak[:-1]
            ...
            if infix:
                ...
                for i in range(0, len(infix)):
                    stak = []
                    for j in range(0, len(infix[i])):
                        if (infix[i][j] == '+') or (infix[i][j] == '-') or (infix[i][j] == '*') or (infix[i][j] == '/'):
                            if len(stak) > 1:
                                tmp = self.Check_is_valid_data(stak[len(stak) - 1])
                                tmp1 = self.Check_is_valid_data(stak[len(stak) - 2])
                                if (tmp == -1) or (tmp1 == -1):
